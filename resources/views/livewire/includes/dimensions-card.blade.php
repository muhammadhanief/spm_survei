<div id="dimensions-list">
    @foreach ($dimensions as $dimension)
        <div wire:key='{{ $dimension->id }}'
            class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-purple-600 hover:shadow ">
            <div class="flex justify-between space-x-2">
                <h3 class="text-lg text-semibold text-gray-800">{{ $dimension->name }}</h3>
                <div class="flex items-center space-x-2">
                    ...
                    <button wire:click='delete({{ $dimension->id }})'
                        wire:confirm='Apakah Anda yakin ingin menghapus dimensi "{{ $dimension->name }}"?'
            ...
            <span class="text-xs text-gray-500"> Dibuat tanggal {{ $dimension->created_at }} </span>
